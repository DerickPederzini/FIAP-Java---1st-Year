...
public class EXERICIO111 {
	...
	public static void main(String[] args) {
		...
		double valori3 = 0 , valori5 = 0, valori7 = 0, valorF = 0, valorTotal = 0;
		
		
		//atribuição inicial de variável	
		//inicio while	
		...
		//começo do processamento e contas
		...
		while(pagamento != 'V' && pagamento != 'P'){
			System.out.println("Digite um pagamento disponível: ");
			pagamento = e.next().toUpperCase().charAt(0);
		}
		...
		//começo da saída		
				...
				if (pagamento =='V' && quantidade > 2 ){
					...
					System.out.println("Forma de pagamento: "+ pagamento);
					System.out.println("quantia de computadores comprados: "+quantidade);
					System.out.println("Valor da quantia de computadores comprados com 10% de acréscimo: "+(valorF * quantidade)*1.1);
		...
		if (pagamento == 'V' && quantidade > 2  ){
			System.out.println((valorTotal*quantidade)*0.8);
			}else { 
			System.out.println((valorTotal*quantidade)*0.9);
			}
			if (pagamento == 'P'){
			System.out.println((valorTotal*quantidade)*1.1);
		}
